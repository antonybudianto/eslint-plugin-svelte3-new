...
const transform_message = (message, { unoffsets, dedent, offsets, range }) => {
  ...
  if (message.fix) {
    while (
      ...
      transformed_code[message.fix.range[0]] === message.fix.text[0]
    ) {
      message.fix.range[0]++;
    ...
    while (
      message.fix.range[0] < message.fix.range[1] &&
      transformed_code[message.fix.range[1] - 1] ===
        message.fix.text[message.fix.text.length - 1]
    ) {
      message.fix.range[1]--;
  ...
  if (message.fix) {
    if (message.fix.range[0] < range[0]) {
      message.fix.text = message.fix.text.slice(
        range[0] - message.fix.range[0]
      );
...
const preprocess = text => {
  ...
  messages = (ignore_warnings
    ? warnings.filter(warning => !ignore_warnings(warning.code, warning))
    : warnings
  ...
  transformed_code = injected_vars.length
    ? `/* eslint-disable */let ${injected_vars
        .map(v => v.name)
        .join(",")};\n/* eslint-enable */`
    : "";
    ...
    walk(ast.html, {
      enter(node, parent, prop) {
        ...
        if (node.context && typeof node.context === "object") {
          ...
          transformed_code += `/* eslint-disable */{${names
            .map(name => `let ${name}=0;`)
            .join("")}/* eslint-enable */\n`;
        }
...
const is_valid_message = message => {
  switch (message.ruleId) {
    ...
    case "no-restricted-syntax":
      return message.nodeType !== "LabeledStatement";
    case "no-self-assign":
      return false;
...
const linter_path = Object.keys(require.cache).find(
  path =>
...
let Linter;
if (!linter_path) {
  Linter = require("eslint/lib/linter.js");
} else {
...
const get_setting_function = (config, key, default_value) => {
  ...
  return typeof value === "function"
    ? value
    : Array.isArray(value)
    ? Array.prototype.includes.bind(value)
    : () => value;
};
...
Linter.prototype.verify = function(code, config, options) {
  ...
  if (options && options.filename) {
    if (
      get_setting_function(config, "svelte3/enabled", n =>
        n.endsWith(".svelte")
      )(options.filename)
    ) {
      // lint this Svelte file
      options = Object.assign({}, options, { preprocess, postprocess });
      ignore_warnings = get_setting_function(
        config,
        "svelte3/ignore-warnings",
        false
      );
      lint_template = get_setting_function(
        config,
        "svelte3/lint-template",
        () => false
      )(options.filename);
      const ignore_styles = get_setting_function(
        config,
      ...
      const compiler_options_setting = get_setting_function(
        config,
        "svelte3/compiler-options",
        false
      );
      compiler_options = compiler_options_setting
        ? Object.assign(
            { generate: false },
            compiler_options_setting(options.filename)
          )
        : { generate: false };
    }
  }
